...
import stackqueuelinkedlist.ArrayQueue;
import stackqueuelinkedlist.Stack;
...
public class ListGraph {
	...
	public void addEdges(int start, int[] end, int[] weights) {
		...
		for (int j = 0; j < end.length; j++) {
			...
			node.next = new ListGraphNode(i, end[j], weights[i], null);
			node = node.next;
		}
	}
	...
	public void dijkstra(int start) {
		...
		int x = value2index.get(start); // 根据哈希表获得对应的下标
		if (x == -1) {
			throw new RuntimeException("未找到顶点");
		}
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = 0; j < length; j++) {
				// 1.只修正未找到最短路径的
				// 2.修正后新顶点需要可达
				// 3.如果使用新的最短路径比原有路径短，或者以前不可达，使用新的最短路径
				// 符合上面三点，则可以修正路径
				if (s[j] != 1 && distance[v][j] != 0
						&& (min + distance[x][j] < distance[x][j] || distance[x][j] == 0)) {
					// 说明加入了中间顶点之后找到了更短的路径
		...
		for (int i = 0; i < length; i++) {
			if (distance[x][i] != 0) {
				...
				// 由于Path存储的特性，可以逆序输出路径，如果有兴趣，则可以用栈正序输出
